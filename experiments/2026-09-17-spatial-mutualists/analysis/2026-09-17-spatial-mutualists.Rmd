...
experiment_slug <- "2026-09-17-spatial-mutualists"
...
```{r}
host_parent_num_task_sets_env_together_plt <- time_data %>%
  filter(update %% 10000 == 0 & sym_condition != "no-syms") %>%
  ggplot(
    aes(
      x = update,
      y = CurUpdate_host_parent_num_task_sets,
...
ggsave(
  file = paste(plot_dir, "host_parent_num_task_sets_env_together_plt.pdf", sep = "/"),
  plot = host_parent_num_task_sets_env_together_plt,
  width = 15,
  height = 8
)
```
...
```{r}
host_parent_entropy_task_sets_env_together_plt <- time_data %>%
  filter(update %% 10000 == 0) %>%
  ggplot(
    aes(
      x = update,
...
ggsave(
  file = paste(plot_dir, "host_parent_entropy_task_sets_env_together_plt.pdf", sep = "/"),
  plot = host_parent_entropy_task_sets_env_together_plt,
  width = 15,
  height = 8
)
```
